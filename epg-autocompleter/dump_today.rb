...
def generateAndProcess()
  ...
  q = "select 
programid,starttime,endtime, channel.callsign,title,programuri,description,subtitle,channum from channelscan_channel,channel, program where 
...
#{crid}|#{dvb}|#{ds}|#{de}|#{chnum}|#{ch}|#{title}|#{stt}|#{desc}"
  end  
